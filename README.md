...
sudo apt install build-essential gcc make perl dkms git
...
## Problems so far
1. The pose generated by the vMF algorithm is not always located near the angle grinder's center of gravity. Instead, it may near the top, which could lead to the failure of grasping. 

## What to do next?
1. Set a height limit to the prediction of grasping pose, so that those poses near the top would be filtered out.
1. Algorithm for placement.
